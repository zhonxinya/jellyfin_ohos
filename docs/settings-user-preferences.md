...
「插件」页原来只能在**已经配好的仓库**里装插件，想加第三方仓库（很多插件不在官方仓库里）
必须去 Web 控制台。现在插件页多了一行「插件仓库」入口，进新页面可以：

- 看当前仓库（名称 + 地址 + 启用开关）；
- 添加仓库（名称 + 地址）、删除仓库、单独开关某一个仓库；
- 保存时**整份列表**提交（服务端就是这个语义：`POST /Repositories` 直接替换
  `ServerConfiguration.PluginRepositories`）。

两个刻意的设计：

- 新增的仓库**默认不启用** —— 地址填错时服务端不会立刻去拉目录、也不会刷一屏错误日志，
  用户确认无误后再打开开关；
- 地址必须是 `http(s)://` 开头、且不能与已有条目重复，这两条在界面上就地拦下。

NAPI 侧只是把 `/Repositories` 加进管理路径白名单（`GET` 读、`POST` 整体替换）。

设备实测（全部可逆）：

```
[OK ] 显示当前仓库名称/地址、“与服务器一致”
[OK ] 恒等保存后服务端列表不变 [{'Name': 'Jellyfin Stable', …, 'Enabled': True}]
     > 仓库名称 = probe-repo，仓库地址 = https://example.invalid/manifest.json
[OK ] 列表里出现新仓库（提示未启用）
[OK ] 服务端多出这个仓库 [{'Name': 'Jellyfin Stable', …}, {'Name': 'probe-repo', …, 'Enabled': False}]
[OK ] 新增的仓库默认不启用 False
[OK ] 点「删除」后列表里只剩官方仓库
[OK ] 服务端回到原来的列表 [{'Name': 'Jellyfin Stable', …, 'Enabled': True}]
```

> 第一次跑这段验收时脚本点错了行（删掉了官方仓库那一行）—— 服务端当时确实被删了，
> 已按原值写回并核对逐字节一致。教训是：列表里同名按钮要按**行**定位，不能取第一个匹配。
>
> 顺带一提，用 `probe-repo` 这种**默认不启用**的假地址做验证是刻意的：
> 服务端不会去拉一个关着的仓库，也就不会在日志里留下抓取失败的噪声。
